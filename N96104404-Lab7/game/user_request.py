...
class TowerDeveloper:
    ...
    def update(self, user_request: str, model):
        ...
        if user_request == "upgrade" and model.selected_tower.level < 5:  #塔最多只能升到五等
             if model.money >= model.selected_tower.level:  #錢大於可以升級塔的價格,則可以升級一級
                model.selected_tower.level += 1 
                model.money -= 100  # 升級一次付100
